<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('article.create') }}" class="border border-green-500 hover:bg-yellow-500 hover:text-white px-4 py-2 rounded-md">CREATE</a>
        </div>    
    </x-slot>
                            ...
                            @foreach ($articles as $article)
                                <tr>
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                        ...
                                        <a href="{{ route('article.edit', $article->id) }}" class="border border-slate-yellow-500 hover:bg-yellow-500 hover:text-white px-4 py-2 rounded-md">EDIT</a>
                                        {{-- add delete button using form tag --}}
                                        <form method="post" action="{{ route('article.destroy', $article->id) }}" class="inline">
                                            @csrf
                                            @method('delete')
                                            <button class="border border-slate-red-500 hover:bg-red-500 hover:text-white px-4 py-2 rounded-md">DELETE</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
